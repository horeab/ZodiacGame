...
public class CampaignLevelEnumService {
    ...
    public LettersGameButtonSkin getButtonSkin() {
        return EnumUtils.getEnumValue(LettersGameButtonSkin.class, "CAMPAIGN_LEVEL_" + getCategory());
    }

    public QuestionConfig getQuestionConfig() {
        QuestionDifficulty difficulty = (QuestionDifficulty) EnumUtils.getEnumValue(LettersGame.getInstance().getSubGameDependencyManager().getQuestionDifficultyTypeEnum(), "_" + getDifficulty());
        QuestionConfig questionConfig;
        if (getCategory() != null) {
            QuestionCategory category = (QuestionCategory) EnumUtils.getEnumValue(LettersGame.getInstance().getSubGameDependencyManager().getQuestionCategoryTypeEnum(), "CAT" + getCategory());
